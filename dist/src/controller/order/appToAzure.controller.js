...
exports.appToAzure = void 0;
const index_1 = require("../../db/index");
const date_1 = require("../../utils/date");
const appToAzure = async (req, res) => {
    ...
    `;
        const headerValues = [
            ...
            details.order_type,
            details.discount,
            details.delivery_datetime ? (0, date_1.toPostgresDateTime)(details.delivery_datetime) : null,
            details.order_state,
        ...
        for (const item of order.items) {
            ...
            if (!isDummyItem) {
                const cgst = item.taxes?.find((t) => t.title === "CGST") || {};
                const sgst = item.taxes?.find((t) => t.title === "SGST") || {};
        ...
        `;
            ...
            for (const [index, option] of (item.options_to_add || []).entries()) {
                const cgst = option.taxes?.find((t) => t.title === "CGST") || {};
                const sgst = option.taxes?.find((t) => t.title === "SGST") || {};
    ...
    `;
        const statusValues = [
            ...
            (0, date_1.toPostgresDateTime)(details.created),
            currentUser,
            currentUser,
        ];
