...
const upToAzure = async (req, res) => {
    ...
    `;
        ...
        const headerValues = [
            ...
            details.order_type,
            details.discount,
            (0, date_1.toPostgresDateTime)(details.delivery_datetime),
            details.order_state,
            ...
            details.expected_pickup_time
                ? (0, date_1.toPostgresDateTime)(details.expected_pickup_time)
                : null,
        ];
        await client.query(orderHeaderQuery, headerValues);
        // 2️⃣ Insert into order_line
        async function insertOrderLine({ item, subItem, details, lineNo, parentLineNo = 0, indent = 0, }) {
      ...
      `;
            ...
            const cgst = taxes.find((t) => t.title === "CGST") || {};
            const sgst = taxes.find((t) => t.title === "SGST") || {};
        ...
        for (const item of order.items) {
            ...
            if (item.options_to_add?.length > 0) {
                if (isDummyItem) {
                    const selectSizeOption = item.options_to_add.find((opt) => opt.group?.title?.trim()?.toLowerCase() === "select size");
    ...
    `;
        const statusValues = [
            ...
            null,
            order.store.id,
            (0, date_1.toPostgresDateTime)(details.updated),
            currentUser,
            currentUser,
        ];
