...
const orderStatus = async (req, res) => {
    try {
        ...
        const orderCheck = await index_1.pool.query(`SELECT 1 FROM "OOMiddleware".order_header WHERE order_id = $1`, [orderId]);
        ...
        const latestRes = await index_1.pool.query(`SELECT * 
        ...
        if (latest) {
            ...
            if (incomingNum !== latestNum + 1) {
                const allowedNext = Object.keys(STATUS_ORDER).find((k) => STATUS_ORDER[k] === latestNum + 1) ?? "none";
        ...
        const colRes = await index_1.pool.query(`SELECT column_name
         FROM information_schema.columns
        WHERE table_schema = $1 
          AND table_name = $2 
          AND column_name = $3`, ["OOMiddleware", "order_new_status", "prev_status"]);
      ...
      `;
        ...
        const insertRes = await index_1.pool.query(insertQuery, insertValues);
